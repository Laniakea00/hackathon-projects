...
async def run_evaluation(
    ...
    print_files_max: int = 60,
) -> list[EvaluationResult]:
    """Run evaluation on JSON files in the dataset directory."""
    console = Console()
    ...
    # --- used files support (avoid repeats between runs)
    used: set[str] = set()
    if used_file:
        used_file.parent.mkdir(parents=True, exist_ok=True)
        if used_file.exists():
            used = set(x.strip() for x in used_file.read_text(encoding="utf-8").splitlines() if x.strip())

        # exclude already used files
        json_files = [p for p in json_files if p.name not in used]

        if not json_files:
            console.print(f"[yellow]All files are already used (from {used_file}). Clearing used list.[/yellow]")
    ...
    if print_files:
        console.print("\n[bold cyan]Selected files:[/bold cyan]")
        to_show = json_files[: max(0, print_files_max)]
        for p in to_show:
            console.print(f"  [dim]• {p.name}[/dim]")
        if len(json_files) > len(to_show):
            console.print(f"  [dim]... and {len(json_files) - len(to_show)} more[/dim]")
    ...
    console.print(
        Panel(
            ...
            f"Limit: [yellow]{limit or 'ALL'}[/yellow]\n"
            f"Seed: [yellow]{seed if seed is not None else 'None'}[/yellow]\n"
            f"Used-file: [yellow]{str(used_file) if used_file else 'None'}[/yellow]",
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        with Progress(
            ...
            task = progress.add_task("[cyan]Evaluating protocols...", total=len(json_files))

            async def process_file(json_file: Path):
                try:
                    result = await evaluate_single(client, endpoint, json_file, semaphore)
    ...
    # persist used files only if run succeeded for those files (even with errors ok)
    if used_file:
        new_used = [r.protocol_id for r in results]  # protocol_id is inside file, not filename
        # лучше сохранять именно filenames, поэтому сохраняем имена выбранных json_files:
        picked_names = [p.name for p in json_files]
        prev = set()
        if used_file.exists():
            prev = set(x.strip() for x in used_file.read_text(encoding="utf-8").splitlines() if x.strip())
        merged = list(prev.union(picked_names))
        merged.sort()
...
def write_metrics_json(submission_name: str, metrics: dict, output_path: Path):
    ...
    output_data = {"submission_name": submission_name, **metrics}
    with open(output_path, "w", encoding="utf-8") as f:
...
Examples:
  ...
  python main.py -e http://api.example.com/diagnose -d ./protocols -n team_alpha -p 10
    ...
    parser.add_argument(
        ...
        default=0,
        help=
        "Evaluate only N random files (0 = all)")
    parser.add_argument(
        "--seed",
        type=int,
        default=None,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a text file storing already used filenames (avoid repeats across runs)",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
        help="Shuffle dataset files before sampling (recommended when using --limit)",
    )
    parser.add_argument(
        "--print-files",
        action="store_true",
        help="Print selected filenames used in this run",
    )
    parser.add_argument(
        "--print-files-max",
        type=int,
        default=60,
        help="Max number of filenames to print when --print-files is set",
    )
    ...
    results = asyncio.run(
        run_evaluation(
            ...
            print_files_max=args.print_files_max,
        )
    )
